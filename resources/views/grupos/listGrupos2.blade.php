    ...
    async searchContent(event) {
        ...
        switch (type) {
            case 'grupo':
                ...
                setTimeout(async () => {
                    $('#sectorGroup').load(`{{ route('sectorsByGroup') }}?id=${id}`, () => {
            ...
            case 'sector':
                ...
                setTimeout(async () => {
                    $('#temasSectorGroup').load(`{{ route('temasBySector') }}?id=${id}`, () => {
                        this.openTemas = true;
                    });
                }, 300);
            break;
            case 'tema':
                ...
                setTimeout(async () => {
                    $('#cuadrosEstadisticos').load(`{{ route('cuadrosEstadisticosByTema') }}?id=${id}`, () => {
        ...
        <ul class="flex-column space-y space-y-6 text-sm font-medium text-gray-500 md:me-4 mb-4 md:mb-0">
            <li>
                <label for="grupo_2" class="inline-flex items-center px-4 py-3 bg-white border-l-4 border-orange-500 rounded-lg w-full transition ease-in-out delay-75 hover:translate-y-1 hover:scale-110 hover:text-orange-400 cursor-pointer has-[:checked]:border-orange-500 has-[:checked]:ring-1 has-[:checked]:ring-orange-500 has-[:checked]:text-orange-400">
                    ...
                    <input type="radio" name="grupo_id" id="grupo_2" value="2" class="sr-only" @click="searchContent($event)" />
                </label>
            </li>
            <li>
            ...
            <li>
                <label for="grupo_32" class="inline-flex items-center px-4 py-3 bg-white border-l-4 border-green-500 rounded-lg w-full transition ease-in-out delay-75 hover:translate-y-1 hover:scale-110 hover:text-green-400 cursor-pointer has-[:checked]:border-green-500  has-[:checked]:ring-1 has-[:checked]:ring-green-500 has-[:checked]:text-green-400">
                    ...
                    <input type="radio" name="grupo_id" id="grupo_32" value="32" class="sr-only" @click="searchContent($event)" />
                </label>
            </li>
            <li>
                <label for="grupo_37" class="inline-flex items-center px-4 py-3 bg-white border-l-4 border-fuchsia-500 rounded-lg w-full transition ease-in-out delay-75 hover:translate-y-1 hover:scale-110 hover:text-fuchsia-400 cursor-pointer has-[:checked]:border-fuchsia-500  has-[:checked]:ring-1 has-[:checked]:ring-fuchsia-500 has-[:checked]:text-fuchsia-400">
                    ...
                    <input type="radio" name="grupo_id" id="grupo_37" value="37" class="sr-only" @click="searchContent($event)" />
                </label>
            </li>
        </ul>
