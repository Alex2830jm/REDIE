    ...
    <div class="px-4 py-6 mb-8 bg-white rounded-lg shadow-md space-y-6">
        <div class="flex justify-between mt-2">
            <h1 class="px-4 py-2 text-2xl font-semibold text-gray-700 flex items-center">
                ...
                Editar datos del usuario: {{ $user->name }} 
            </h1>
                    ...
                    <form action="{{ route('usuarios.update', $user->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="mb-5">
                                ...
                                <input type="text" name="primerApellido" id="primerApellido" value="{{ $user->primerApellido }}"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            </div>
                                ...
                                <div class="flex flex-row justify-between gap-3">
                                    @foreach ($roles as $role)
                                        <label for="{{ $role->name }}"
                                            ...
                                            <input type="radio" name="role_id" value="{{ $role->name }}" {{ $user->roles->first()->id == $role->id ? 'checked' : '' }}
