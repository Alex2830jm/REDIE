<x-dashboard-layout>
    <div x-data="{
        ...
        async filesUpload(event) {
            ...
            this.yearPost = $(`input[name='yearPost']`).val();
            console.log(files);
            ...
            let fileDataArray = newFiles.map(file => {
                let fileData = {
                    url: URL.createObjectURL(file),
                    name: file.name,
                    preview: ['xlsx', 'xls', 'cvs'].includes(file.name.split('.').pop().toLowerCase()),
                    size: file.size > 1024 ?
                        file.size > 1048576 ?
                        ...
                        echo '
                            <div>
                                <label for="year_' . $i . '" class="flex cursor-pointer items-center justify-center rounded-md border border-gray-300 bg-gray-100 px-3 py-2 text-gray-900 hover:border-gray-400 has-[:checked]:border-blue-500 has-[:checked]:bg-blue-500 has-[:checked]:text-white">
                                    <input type="radio" name="yearPost" value="' . $i . '" id="year_' . $i . '" class="sr-only" />
                                        ...
                                        <div class="flex items-center gap-x-3">
                                            ...
                                            <div>
                                                <p class="flex text-sm font-medium text-gray-800 justify-between">
                                                    ...
                                                    <span class="truncate inline-block max-w-[300px] align-bottom"
                                                        x-text="document.yearPost"></span>
                                                </p>
                                                <p class="text-xs text-justify text-gray-500">
                                                    Plazas autorizadas, registradas en el sistema de nómina de la
                                                    Dirección General de Desarrollo y Administración de Personal
                                                </p>
                                            </div>
                                        </div>
